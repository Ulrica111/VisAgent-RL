...
from verl.models.transformers.qwen2_vl import get_rope_index as get_rope_index_qwen2vl
from verl.models.transformers.qwen3_vl import get_rope_index as get_rope_index_qwen3vl
...
def _mm_inputs_carry_visual_pixels(mm) -> bool:
    """Whether trajectory already carries visual tensors (prompt images or merged turns). Used for batch filter ``hasimage``."""
    if not isinstance(mm, dict) or len(mm) == 0:
        return False
    pv = mm.get("pixel_values")
    if pv is not None:
        if isinstance(pv, torch.Tensor):
            return bool(pv.numel() > 0)
        try:
            return len(pv) > 0
        except TypeError:
            return True
    pvv = mm.get("pixel_values_videos")
    if pvv is not None:
        if isinstance(pvv, torch.Tensor):
            return bool(pvv.numel() > 0)
        try:
            return len(pvv) > 0
        except TypeError:
            return True
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params, max_turn_of_validation=None):
    ...
    for i in range(batch_size):
        for _ in range(sampling_params.n):
            ...
            hasimage_list.append(_mm_inputs_carry_visual_pixels(multi_modal_inputs[i]))
        ...
        for idx, obs, act, rew, done, is_error in zip(active_indices, observations, actions, rewards, dones, is_errors):
            ...
            if 'prompt_token_ids_vllm' in obs.keys() and 'prompt_token_ids_model' in obs.keys():
                ...
                if mm_input:
                    ...
                    if _mm_inputs_carry_visual_pixels(mm_input_list[idx]):
                        hasimage_list[idx] = True
    ...
    if processor is not None and (
        processor.image_processor.__class__.__name__ in (
            "Qwen2VLImageProcessor", "Qwen2VLImageProcessorFast",
            "Qwen3VLImageProcessor", "Qwen3VLImageProcessorFast",
        )
    ):
        # Select rope index function based on model family
        _is_qwen3vl = processor.image_processor.__class__.__name__ in (
            "Qwen3VLImageProcessor", "Qwen3VLImageProcessorFast"
        )
        get_rope_index = get_rope_index_qwen3vl if _is_qwen3vl else get_rope_index_qwen2vl
