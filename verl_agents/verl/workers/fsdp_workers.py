...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        def _looks_like_vlm(cfg) -> bool:
            mt = (getattr(cfg, "model_type", None) or "").lower().replace("-", "_")
            if not mt:
                return getattr(cfg, "vision_config", None) is not None
            if "vl" in mt or "llava" in mt or "internvl" in mt:
                return True
            return getattr(cfg, "vision_config", None) is not None
        ...
        with init_context(), warnings.catch_warnings():
            ...
            vision_mapping = getattr(AutoModelForVision2Seq, "_model_mapping", None) if AutoModelForVision2Seq else None
            if vision_mapping is not None and type(actor_model_config) in vision_mapping.keys():
                actor_module_class = AutoModelForVision2Seq
            elif getattr(actor_model_config, "model_type", None) == "qwen3_vl":
                # AutoModel resolves to Qwen3VLModel (no lm_head); PPO log_prob needs logits from ForConditionalGeneration.
                from transformers.models.qwen3_vl.modeling_qwen3_vl import Qwen3VLForConditionalGeneration

                actor_module_class = Qwen3VLForConditionalGeneration
                if self.rank == 0:
                    logger.warning(
                        "AutoModelForVision2Seq mapping missing: loading Qwen3VLForConditionalGeneration for FSDP actor "
                        "(AutoModel/Qwen3VLModel has no logits)."
                    )
            elif getattr(actor_model_config, "model_type", None) == "qwen3_vl_moe":
                from transformers.models.qwen3_vl_moe.modeling_qwen3_vl_moe import (
                    Qwen3VLMoeForConditionalGeneration,
                )

                actor_module_class = Qwen3VLMoeForConditionalGeneration
                if self.rank == 0:
                    logger.warning(
                        "AutoModelForVision2Seq mapping missing: loading Qwen3VLMoeForConditionalGeneration for FSDP actor."
                    )
            elif _looks_like_vlm(actor_model_config):
                from transformers import AutoModel

                actor_module_class = AutoModel
                if self.rank == 0:
                    logger.warning(
                        "AutoModelForVision2Seq unavailable; loading VLM via AutoModel (upgrade transformers "
                        ">= 4.45 with vision2seq helpers for preferred path)."
                    )
            else:
...
#############################################################################################################
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            # vLLM init profiles KV as total*gpu_memory_utilization - peak_torch_mem; FSDP actor on
            # the same GPU inflates peak and can make available_kv <= 0 ("No available memory for
            # the cache blocks"). Offload FSDP to CPU for the vLLM LLM() constructor, then restore.
            if self._is_offload_param:
                offload_fsdp_model_to_cpu(self.actor_module_fsdp)
                torch.cuda.empty_cache()
                log_gpu_memory_usage("After offload FSDP before vLLM rollout init", logger=logger)
            self.rollout, self.rollout_sharding_manager = self._build_rollout(
                trust_remote_code=self.config.model.get("trust_remote_code", False)
            )
            if self._is_offload_param:
                load_fsdp_model_to_gpu(self.actor_module_fsdp)
                log_gpu_memory_usage("After reload FSDP after vLLM rollout init", logger=logger)
