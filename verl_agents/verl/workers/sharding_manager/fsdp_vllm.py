...
# HF ForConditionalGeneration uses model.visual / model.language_model; merged safetensors + vLLM
# hf_to_vllm_mapper expect `visual.*` and short `model.*` (not model.visual -> language_model.model.visual)
_VISION2SEQ_FSDP_TO_VLLM_KEYS = (
    "Qwen2_5_VLForConditionalGeneration",
    "Qwen2VLForConditionalGeneration",
)


def _remap_vision2seq_fsdp_key_to_merged_safetensors(name: str) -> str:
...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        # 缓解 actor 步后显存碎片，避免第二次起 wake_up 时 CuMem 映射失败
        gc.collect()
    ...
    def update_params(self, updated_params):
        ...
        if model.config.architectures[0] in ["DeepseekV2ForCausalLM", "DeepseekV3ForCausalLM"]:
            ...
            arch0 = model.config.architectures[0] if getattr(model.config, "architectures", None) else None

            def _param_tensor(p):
                return p.full_tensor() if world_size != 1 and hasattr(p, "full_tensor") else p

            if arch0 in _VISION2SEQ_FSDP_TO_VLLM_KEYS:

                def _iter_vision2seq():
                    for name, param in updated_params.items():
                        t = _param_tensor(param)
                        yield (_remap_vision2seq_fsdp_key_to_merged_safetensors(name), t)

                weight_iter = _iter_vision2seq()
            else:
                weight_iter = ((name, _param_tensor(param)) for name, param in updated_params.items())
            loaded_params = model.load_weights(weight_iter)
