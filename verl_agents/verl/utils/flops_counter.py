...
VALID_CONFIG_TYPE = {"llama", "qwen2", "qwen2_vl", "qwen2_5_vl", "qwen3_vl", "deepseek_v3"}


def _dense_lm_config_for_flops(config: PretrainedConfig) -> PretrainedConfig:
    """Qwen3-VL 等多模态顶层 config 将 LLM 维度放在 ``text_config``，无顶层 ``vocab_size``。"""
    tc = getattr(config, "text_config", None)
    if tc is not None and getattr(tc, "hidden_size", None) is not None:
        return tc
    return config
...
class FlopsCounter:
    ...
    def _estimate_qwen2_flops(self, tokens_sum, batch_seqlens, delta_time):
        cfg = _dense_lm_config_for_flops(self.config)
