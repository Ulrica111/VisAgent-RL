...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        if self.processor is not None:
            ...
            proc_min = getattr(img_proc, "min_pixels", None)
            proc_max = getattr(img_proc, "max_pixels", None)
            print(f"min_pixels (image_processor): {proc_min}, dataset config min_pixels: {self.min_pixels}")
            print(f"max_pixels (image_processor): {proc_max}, dataset config max_pixels: {self.max_pixels}")
        else:
            print(f"no processor; dataset min_pixels={self.min_pixels}, max_pixels={self.max_pixels}")
        print("######################################################")
        ...
        _root = config.get("mm_hint_image_root", None)
        self.mm_hint_image_root = os.path.normpath(os.path.expanduser(_root)) if _root else None
    ...
    def _read_files_and_tokenize(self):
        ...
        for data_file_path in self.data_files:
            if "image_val_dataset" in data_file_path:
                # 已显式传入 *val_dataset_processed.json 时直接读该文件，避免误用旧的
                # processed_w_image_hint 侧车缓存（条数与主文件不一致时 tqdm 会一直是旧 batch 数）。
                if data_file_path.endswith("_processed.json"):
                    ...
                    if os.path.exists(save_data_path):
                        ...
                        data_list = json.load(open(data_file_path, "r"))
                        data_list = transfer_to_rl_form_image_w_mm_hint(
                            data_list, self.prompt_template_path
                        )
                        save_data_path = data_file_path.replace("image_val_dataset", "processed_w_image_hint")
    ...
    def _apply_sample_limit(self):
        if self._sample_limit is None:
            return
        try:
            n = int(self._sample_limit)
        except (TypeError, ValueError):
            return
        if n <= 0:
            return
        orig = len(self.dataframe)
        self.dataframe = self.dataframe[:n]
        print(
            f"sample_limit={n}: using first {len(self.dataframe)} of {orig} samples (original order, truncated)"
        )
    ...
    def _resolve_mm_hint_image_path(self, hint_path: str) -> str:
        """Join relative hint_path to mm_hint_image_root; remap broken abs paths under verl_agents."""
        if not self.mm_hint_image_root or not isinstance(hint_path, str):
            return hint_path
        root = self.mm_hint_image_root
        if not os.path.isabs(hint_path):
            return os.path.join(root, hint_path)
        if os.path.isfile(hint_path):
            return hint_path
        norm = hint_path.replace("\\", "/")
        for marker in ("verl_agents/", "PyVision-RL/verl_agents/"):
            if marker in norm:
                suffix = norm.split(marker, 1)[1]
                cand = os.path.join(root, suffix)
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            if hint_type == "image":
                image_path = self._resolve_mm_hint_image_path(row_dict['mm_hint']['hint_path'])
