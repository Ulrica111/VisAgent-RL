...
class Tracking(object):
    ...
    def __init__(self, 
        ...
        self._verl_closed = False

    def close(self):
        """Synchronous teardown for W&B / SwanLab. Prefer calling before process exit (e.g. end of trainer.fit)."""
        if getattr(self, "_verl_closed", False):
            return
        self._verl_closed = True
        ...
        if "swanlab" in self.logger:
            try:
                ...
                pass
            self.logger.pop("swanlab", None)
        if "vemlp_wandb" in self.logger:
            try:
                self.logger["vemlp_wandb"].finish(exit_code=0)
            except Exception:
                pass
            self.logger.pop("vemlp_wandb", None)
