...
def get_hf_max_position_embeddings(config: PretrainedConfig) -> int:
    """Max context length from a HF config. VLMs (e.g. Qwen3-VL) store it on ``text_config``, not the root."""
    mp = getattr(config, "max_position_embeddings", None)
    if isinstance(mp, int) and mp > 0:
        return mp
    text_cfg = getattr(config, "text_config", None)
    if text_cfg is not None:
        mp = getattr(text_cfg, "max_position_embeddings", None)
        if isinstance(mp, int) and mp > 0:
            return mp
    raise AttributeError(
        f"{type(config).__name__}: could not resolve max_position_embeddings (not on root or text_config)"
    )
