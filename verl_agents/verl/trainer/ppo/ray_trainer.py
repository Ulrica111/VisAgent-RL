...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        def _build_dataset(data_files, sample_limit):
            kwargs = {
            ...
            if dataset_cls in (RLHFDataset, RLHF_wo_mm_hint_Dataset):
                kwargs["sample_limit"] = sample_limit
            return dataset_cls(**kwargs)

        self.train_dataset = _build_dataset(
            self.config.data.train_files,
            self.config.data.get("max_train_samples", None),
        )
        ...
        self.val_dataset = _build_dataset(
            ...
            self.config.data.get("max_val_samples", None),
        )
    ...
    @staticmethod
    def _jsonify_metric_value(val):
        if isinstance(val, dict):
            return {k: RayPPOTrainer._jsonify_metric_value(v) for k, v in val.items()}
        if isinstance(val, (list, tuple)):
            return [RayPPOTrainer._jsonify_metric_value(v) for v in val]
    ...
    def _maybe_dump_val_metrics_json(self, val_metrics: dict) -> None:
        dump_dir = self.config.trainer.get("val_metrics_json_dir", None)
        if not dump_dir:
            return
        os.makedirs(dump_dir, exist_ok=True)
        path = os.path.join(dump_dir, f"val_metrics_gs{int(self.global_steps)}.json")
        payload = {
            ...
            "metrics": {k: self._jsonify_metric_value(v) for k, v in val_metrics.items()},
        }
        with open(path, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
        print(f"Wrote validation metrics to {path}")
    ...
    def _validate(self):
        ...
        if not data_source_lst:
            print("[validation] skipped: val dataloader yielded 0 batches (empty val_files / val dataset).")
            return {}
    ...
    def fit(self):
        ...
        if self.config.trainer.get("val_before_train", True):
            if self.config.actor_rollout_ref.rollout.val_kwargs.eval_enable:
                ...
                self._maybe_dump_val_metrics_json(val_metrics)
                logger.close()
                return
            else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            while True:
                ...
                metrics = {}
                extra_filtering_config = {
                    "end_reason_filter_reserve_names": self.config.algorithm.filter_groups.get(
                        "end_reason_filter_reserve_names", None
                    ),
                }
                timing_raw = {}
                ...
                dbg_rollout_load = self.config.trainer.get("debug_train_load_rollout_path") or None
                if dbg_rollout_load:
                    dbg_rollout_load = (
                        dbg_rollout_load if os.path.isabs(dbg_rollout_load) else os.path.abspath(dbg_rollout_load)
                    )
                    if not os.path.isfile(dbg_rollout_load):
                        raise FileNotFoundError(
                            f"trainer.debug_train_load_rollout_path not found or not a file: {dbg_rollout_load}"
                        )
                    print(
                        f"[trainer] Debug: skip rollout for this step, loading DataProto from {dbg_rollout_load}",
                        flush=True,
                    )

                # Generate and filter rollouts until we have enough (unless loading cached batch)
                with _timer("step", timing_raw):
                    if dbg_rollout_load:
                        batch = DataProto.load_from_disk(dbg_rollout_load)
                        batch.check_consistency()
                        accumulated_rollout_count = len(batch)
                        num_gen_batches_for_this_step = 0
                    if not dbg_rollout_load:
                    ...
                    if self.config.trainer.balance_batch and not dbg_rollout_load:
                        self._balance_batch(batch, metrics=metrics)

                    first_dbg_dir = self.config.trainer.get("the_first_batch_rollout_data_dir", None)
                    if (
                        first_dbg_dir
                        and not dbg_rollout_load
                        and not getattr(self, "_verl_saved_debug_train_batch_once", False)
                    ):
                        os.makedirs(first_dbg_dir, exist_ok=True)
                        _dbg_pickle = os.path.join(first_dbg_dir, "debug_train_batch.pkl")
                        batch.save_to_disk(_dbg_pickle)
                        print(
                            f"[trainer] Saved DataProto for training-stage debug reload to {_dbg_pickle} "
                            f'(Hydra override: trainer.debug_train_load_rollout_path="{_dbg_pickle}")',
                            flush=True,
                        )
                        self._verl_saved_debug_train_batch_once = True
                ...
                if oversample_data_pool is not None:
                    metrics.update(
                        compute_data_metrics_oversample_pool(batch=oversample_data_pool, use_critic=self.use_critic)
                    )
                    metrics.update(
                        compute_end_reason_metrics_oversample_pool(
                            batch=oversample_data_pool, extra_filtering_config=extra_filtering_config
                        )
                    )
