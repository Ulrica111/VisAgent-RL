...
parser.add_argument(
    ...
    type=str,
    required=True,
    help=r"The path for your saved model. For megatron, point to the base dir of model, rng, optimizer checkpoints, commonly be `config.default_local_dir/global_step_{global_step}`.",
)
...
def copy_hf_tokenizer_and_processor(hf_src: str, hf_dst: str) -> None:
    """FSDP merge 只写出权重；从基座 HF 拷贝 tokenizer 与多模态 processor，供 vLLM / from_pretrained 加载。"""
    ...
    for name in names:
        ...
        if os.path.isfile(src):
            shutil.copy2(src, os.path.join(hf_dst, name))
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    del state_dict
    del model
    copy_hf_tokenizer_and_processor(args.hf_model_path, hf_path)
