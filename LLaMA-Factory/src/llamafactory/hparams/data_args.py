...
class DataArguments:
    ...
    skip_long_examples: bool = field(
        default=True,
        metadata={
            "help": (
                "Drop supervised examples whose tokenized length exceeds `cutoff_len` instead of keeping them. "
                "Keeping overlong multimodal samples can later be truncated at batch time while image grids stay "
                "full-sized, which breaks Qwen-VL style RoPE (shape mismatch). Set to `false` only if you know "
                "your pipeline never emits longer sequences than `cutoff_len`."
            )
        },
    )
    skip_mm_examples_reaching_cutoff: bool = field(
        default=True,
        metadata={
            "help": (
                "If true (and `skip_long_examples` is true), also drop supervised examples that have images or videos "
                "and whose tokenized length is greater than **or equal to** `cutoff_len`. VL training uses "
                "`infer_seqlen` truncation that can cut inside vision placeholders while collators still attach "
                "full `image_grid_thw`, which triggers Qwen-VL RoPE shape errors (e.g. 8287 vs 8192). "
                "Set to `false` only if your data never hits the cutoff boundary cleanly."
            )
        },
    )
